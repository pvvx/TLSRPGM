...
unsigned char uart_TxIndex[2] = {0};
...
unsigned char uart_RxIndex[2] = {0};
/**
...
void uart_send_dma(uart_num_e uart_num,unsigned char* Addr)
{
    ...
    */
    ...
    if(uart_num == UART0){
        ...
        reg_dma11_addrl =  (unsigned char)((unsigned int)Addr);
        reg_dma11_addrh  = (unsigned char)((unsigned int)Addr>>8);
        reg_dma11_size = 0xff;
        reg_dma_chn_en_h |= (FLD_DMA_CHN_UART1_TX&0xff);
        reg_dma_rdy0_h     |= (FLD_DMA_CHN_UART1_TX&0xff);
    }
...
void uart_recbuff_init(uart_num_e uart_num,unsigned char *RecvAddr, unsigned short RecvBufLen)
{
    ...
    if(uart_num == UART0){
        ...
        reg_dma10_addrl =  (unsigned char)((unsigned int)addr);
        reg_dma10_addrh  = (unsigned char)((unsigned int)addr>>8);
        reg_dma10_size = bufLen; //set receive buffer size
        reg_dma10_mode = FLD_DMA_WR_MEM;   //set DMA 0 mode to 0x01 for receive
        reg_dma_chn_en_h |= (FLD_DMA_CHN_UART1_RX&0xff);
    }
...
void uart_set_rts(uart_num_e uart_num,unsigned char Enable, UART_RTSModeTypeDef Mode, unsigned char Thresh, unsigned char Invert,  GPIO_PinTypeDef pin)
{
    if (Enable)
    {
        if(uart_num == UART0){
            ...
            gpio_set_func(pin,UART1_RTS );//enable rts pin
        }
...
void uart_set_cts(uart_num_e uart_num,unsigned char Enable, unsigned char Select,GPIO_PinTypeDef pin)
{
    if (Enable)
    {
        ...
        if(uart_num == UART0){
            ...
            gpio_set_func(pin,UART1_CTS_I);//enable cts pin
        }
...
{
    //PA1 PA2 supports only uart0 but not uart1
...
void uart_set_rtx_pin(uart_num_e uart_num,GPIO_PinTypeDef rtx_pin)
{
    ...
    if(uart_num == UART0){
        ...
        gpio_set_func(rtx_pin,UART1_RTX);
    }
